...
def autodetect_metric_cols(df: pd.DataFrame):
    ...
    # CSAT
    csat_candidates = [c for c in df.columns if c.upper().startswith("DIM6.1")] + \
                      [c for c in df.columns if "CSAT" in c.upper()] + \
                      [c for c in df.columns if "HAPP" in c.upper()]  # happiness
    csat_col = csat_candidates[0] if csat_candidates else None

    # CES / Value / Fees
    ces_candidates = [c for c in df.columns if c.upper().startswith("DIM6.2")] + \
                     [c for c in df.columns if "CES" in c.upper()] + \
                     [c for c in df.columns if "VALUE" in c.upper()] + \
                     [c for c in df.columns if "FEES" in c.upper()]  # إذا تغيرت التسمية لرسوم
    ces_col = ces_candidates[0] if ces_candidates else None

    # NPS
    nps_candidates = [c for c in df.columns if "NPS" in c.upper()] + \
                     [c for c in df.columns if "RECOMMEND" in c.upper()]
...
common_keys = ["GENDER", "SERVICE", "SECTOR", "NATIONALITY", "ACADEMIC", "AGE", "PERIOD", "EMIRATE", "CHANNEL"]
